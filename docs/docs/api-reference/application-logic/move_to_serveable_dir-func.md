...
Moves a file to somewhere inside `.Magic/served-files/`. This is a convenience function that calls `gen_serveable_path()` and `mv()` to move the file to the app's serveable directory. If you want to create a serveable copy of the file instead of moving the file itself, call [`make_serveable_copy()`](/docs/build/docs/api-reference/application-logic/make_serveable_copy-func) instead.
