...
DD_Components.clamp = function(num, min, max) {
    return Math.min(Math.max(num, min), max);
}

DD_Components.isValidNumberString = function(numberString, decimalSep, thousandsSep) {
    // Generated by Claude
    //----------------------
    ...
    const escapedThousandsSep = thousandsSep.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // Split by decimal separator
    const parts = numberString.split(decimalSep);

    // Can have at most 1 decimal separator (0 or 1 decimal parts)
    if (parts.length > 2) {
        return false;
    }
    ...
    // Validate decimal part (if exists) - should only contain digits
    ...
    // Has thousands separator - validate proper usage

    // Remove optional leading sign
    let numPart = integerPart;
    const sign = integerPart[0];
    if (sign === '+' || sign === '-') {
        numPart = integerPart.slice(1);
    }

    // Split by thousands separator
    const groups = numPart.split(thousandsSep);
    ...
    // If there's only 1 group, shouldn't have thousands separator
    // (e.g., "1," or "12," or "123," is invalid)
    if (groups.length === 1) {
        return false;
    }
...
DD_Components.parseNumber = function(numberString, decimalSep, thousandsSep) {
    // Generated by Claude
    //-----------------------
    // Trim whitespace
    numberString = numberString.trim();

    // Escape special regex characters for the separators
    const escapedThousandsSep = thousandsSep.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // Remove all thousands separators
    const withoutThousands = numberString.replace(new RegExp(escapedThousandsSep, 'g'), '');

    // Replace decimal separator with standard dot (if different)
    const standardFormat = decimalSep === '.'
    ? withoutThousands
...
DD_Components.formatNumber = function(number, precision, decimalSep, thousandsSep) {
    // Generated by Claude
    //----------------------
    ...
    let result = (isNegative ? '-' : '') + withThousands;
...
class DD_Input extends HTMLElement {
    ...
    constructor() {
        ...
        function btnIncrement(event) {
            if (thisInput.value != null) {
                ...
                thisInput.value = thisInput.clamp(thisInput.value + signal*step);
                thisInput.maybeNotifyChange();
                thisInput.previousValue = thisInput.value;
            }
        }

        this.increaseButton = document.createElement("dd-button");
        this.increaseButton.innerHTML = "<span>+</span>";
        this.increaseButton.classList.add("dd-increment-button");
        this.increaseButton.setAttribute("dd-direction", "up");
        this.increaseButton.addEventListener("click", btnIncrement);

        this.decreaseButton = document.createElement("dd-button");
        this.decreaseButton.innerHTML = "<span>&ndash;</span>";
        this.decreaseButton.classList.add("dd-increment-button");
        this.decreaseButton.setAttribute("dd-direction", "down");
        this.decreaseButton.addEventListener("click", btnIncrement);
        ...
        this.input.addEventListener("change", function(event) {
            if (thisInput.hasAttribute("dd-ignore-change")) return;

            if (thisInput.getAttribute("type") == "number") {
                if (this.value == "") {
                    thisInput.value = null;
                } else if (thisInput.isValidNumberString(this.value)) {
                    let value = thisInput.parseNumber(this.value);
                    thisInput._value = thisInput.clamp(value);
                } else {
                    thisInput.value = thisInput.previousValue;
                }
            } else {
                if (this.value == "") {
                    thisInput.value = null;
        ...
        this.input.addEventListener("blur", function(event) {
            if (thisInput.getAttribute("type") == "number") {
                thisInput.setAttribute("dd-ignore-change", "");
                thisInput.input.value = thisInput.formatNumber(thisInput.value);
                thisInput.removeAttribute("dd-ignore-change");
            }
        });
        ...
        this.input.addEventListener("focus", function(event) {
            if (thisInput.getAttribute("type") == "number") {
                thisInput.setAttribute("dd-ignore-change", "");
                if (thisInput.value !== undefined && thisInput.value !== null) {
                    thisInput.input.value = thisInput.value;
                }
                thisInput.input.select();
                thisInput.removeAttribute("dd-ignore-change");
            }
        });
    ...
    connectedCallback() {
        ...
        if (this.getAttribute("type") == "number") {
            ...
            if (!this.hasAttribute("dd-step")) {
                this.setAttribute("dd-step", "1");
            }

            this.increaseButton.classList.add("dd-show");
            this.decreaseButton.classList.add("dd-show");
    ...
    clamp(value) {
        if (this.hasAttribute("dd-min")) {
            const min = parseFloat(this.getAttribute("dd-min"));
            value = Math.max(min, value);
        }
        if (this.hasAttribute("dd-max")) {
            const max = parseFloat(this.getAttribute("dd-max"));
            value = Math.min(max, value);
        }
        return value;
    }

    isValidNumberString(num) {
        return DD_Components.isValidNumberString(num, this.getAttribute("dd-decimal-separator"), this.getAttribute("dd-thousands-separator"));
    }

    parseNumber(num) {
        return DD_Components.parseNumber(num, this.getAttribute("dd-decimal-separator"), this.getAttribute("dd-thousands-separator"));
    }
    ...
    setValue(value) {
        if (this.getAttribute("type") != "number") {
            ...
            this._value = num;
            if (num != null) {
                this.input.value = this.formatNumber(num);
            } else {
                this.input.value = "";
            }
        }
...
style.textContent = `
    ...
    dd-input .dd-increment-button.dd-show {
        display: flex;
    }
